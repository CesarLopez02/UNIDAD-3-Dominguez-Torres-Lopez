        ...
        <div class="row gx-4 gx-lg-5 row-cols-2 row-cols-md-3 row-cols-xl-4 justify-content-center" id="product-list">
            ...
            if ($resultado->num_rows > 0) {
                while ($row = $resultado->fetch_assoc()) {
                    ...
                    echo '<img class="card-img-top" src="data:image/jpeg;base64,' . base64_encode($row['imagen']) . '" alt="' . $row['nombre'] . '" />';
                    echo '<div class="card-body p-4">';
                    echo '<div class="text-center">';
                    echo '<h5 class="fw-bolder">' . $row['nombre'] . '</h5>';
                    echo '<div class="d-flex justify-content-center small text-warning mb-2">';
                    
                    // Generar estrellas (puedes ajustar la cantidad según tus criterios)
                    for ($i = 0; $i < 5; $i++) {
                        echo '<div class="bi-star-fill"></div>'; 
                    }

                    echo '</div>';
                    // Cambiar a un formato de precio, si necesitas un rango, puedes modificarlo
                    echo '$' . number_format($row['precio'], 2);
                    echo '</div>'; // Cierra text-center
